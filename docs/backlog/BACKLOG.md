...
**Status — code complete; infrastructure provisioned; awaiting the first green CI run on `main`.** Everything verifiable without a cloud account has been verified on the real image.
...
**Provisioned by the owner** Railway project `atelier-staging` with Postgres, Redis and an `api` service · health check path `/readyz` · service variables · `RAILWAY_TOKEN`, `RAILWAY_SERVICE`, and the `staging` GitHub environment.
**Remaining** Merge to `main` → CI green → job [9] publishes to GHCR → point the Railway service at the image (it is currently sourced from the GitHub repo, which would bypass the signed artifact) → deploy → generate the domain → set `STAGING_URL`. Ordered walkthrough in [§26 Staging Runbook](../04-engineering/26-staging-runbook.md).
**Still unverified** Zero-downtime replacement and rollback-by-digest. Both need a live deploy; neither can be demonstrated from a development machine.
